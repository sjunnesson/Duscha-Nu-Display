...
bool loadWIFICredsForm() {
  ...
  const char* ssid     = "Duscha Nu WiFi Manager";
  ...
  IPAddress IPRead = WiFi.softAPIP();

  Serial.print("AP IP address: ");
  Serial.println(IPRead);
...
void connectToWiFi() {
  ...
  char ssidBuff[30];
  char passBuff[30];
  ssid.toCharArray(ssidBuff, 30);
  password.toCharArray(passBuff, 30);
...
// check of the wifi reset button is pressed, if so then we reset and load in new
